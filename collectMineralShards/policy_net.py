...
class Policy_net:
    def __init__(self, name: str, temp=0.1):
        ...
        with tf.variable_scope(name):
            ...
            layer_2 = tf.layers.conv2d(inputs=layer_1, filters=256, kernel_size=[3, 3], strides=[1, 1], padding='SAME', activation=tf.nn.relu)
            with tf.variable_scope('policy_net'):
                reshape_spatial = tf.reshape(layer_2, [-1, 16*16*256])
